    ...
    private static void HashBlock(BitArray[] keys, Span<byte> input, bool inverse = false)
    {
        ...
        for(int i = 0; i < 16; i++)
        {
            ...
            BitArray F(BitArray key, BitArray R)
            {
                ...
                for(int i = 0; i < 48; i++)
                    R_expanded[47 - i] = R[32 - E[i]];
                
                var result = key.Xor(R_expanded);
                var newRight = new BitArray(32);
                for (int i = 0; i < 8; i++)
                {
                    var pack = i * 6;
                    byte row = 0;
                    row |= System.Convert.ToByte(result[pack]);
                    row |= (byte)(System.Convert.ToInt32(result[pack + 5]) << 1);

                    byte column = 0;
                    for (var k = 0; k < 4; k++)
                        column |= (byte)(System.Convert.ToInt32(result[pack + k + 1]) << k);

                    var value = SBox[7 - i, row, column];

                    for (var k = 0; k < 4; k++)
                        newRight[i * 4 + k] = System.Convert.ToBoolean(value >> k & 1);
                }

                var FResult = new BitArray(32);
                for (var i = 0; i < 32; i++)
                    FResult[i] = newRight[32 - P[i]];
                return FResult;
            }
        }

        input_bitArray = BitArray_Ext.Unite(input_L, input_R);
        
        var result = new BitArray(64);
        for(int i = 0; i < 64; i++)
            result[63 - i] = input_bitArray[64 - FP[i]];

        var tempByteArray = new byte[8];
        result.CopyTo(tempByteArray, 0);
        tempByteArray.CopyTo(input);
        input.Reverse();
    }
}
